...
use crate::entity::toriel_oven::{MessageTorielOven, TorielOven};
use crate::entity::{
    ActionResult, Entities, Entity, EntityKey, EntityType, Message, MessageContents, StepContext,
};
...
impl Toriel {
    fn d_special_ground_start_action(
        &mut self,
        context: &mut StepContext,
        state: &ActionState,
    ) -> Option<ActionResult> {
        self.player
            .ground_idle_action(context, state)
            .or_else(|| self.d_special_start(context, state))
    }

    fn d_special_air_start_action(
        &mut self,
        context: &mut StepContext,
        state: &ActionState,
    ) -> Option<ActionResult> {
        self.player
            .aerial_action(context, state)
            .or_else(|| self.d_special_start(context, state))
    }
    ...
    fn d_special_start(
        ...
        for oven_key in self.get_ovens(context.entities) {
            ...
            if let Some(TorielOvenAction::Attack) = context
                .entities
                .get(oven_key)
    ...
    fn s_special_ground_start_action(
        &mut self,
        context: &mut StepContext,
        state: &ActionState,
    ) -> Option<ActionResult> {
    ...
    fn n_special_ground_start_action(
        &mut self,
        context: &mut StepContext,
        state: &ActionState,
    ) -> Option<ActionResult> {
