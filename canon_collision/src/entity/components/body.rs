...
pub enum Location {
    ...
    GrabbedLedge {
        ...
        d_y: f32,
        logic: LedgeLogic,
    }, // player.face_right determines which edge on the platform
...
impl Body {
    ...
    fn bps_xy(
        ...
        action_frame: Option<&ActionFrame>,
        state: &ActionState,
    ) -> (f32, f32) {
        self.public_bps_xy(
            context.entities,
            ...
            Location::GrabbedByPlayer(entity_i) => {
                if let Some(player) = entities.get(entity_i) {
                    if let Some(entity_frame) = action_frame {
                        let (grabbing_x, grabbing_y) =
                            player.grabbing_xy(entities, entity_defs, surfaces);
    ...
    pub fn public_bps_xyz(
        ...
        let z = match self.location {
            Location::ItemHeldByPlayer(entity_i) => {
                if let Some(player) = entities.get(entity_i) {
                    player
                        .get_entity_frame(&entity_defs[player.state.entity_def_key.as_ref()])
    ...
    pub fn angle(&self, action_frame: &ActionFrame, surfaces: &[Surface]) -> f32 {
        match self.location {
            Location::Surface { platform_i, .. } if action_frame.use_platform_angle => surfaces
                .get(platform_i)
                .map_or(0.0, |x| x.floor_angle().unwrap_or_default()),
            _ => 0.0,
        }
    }
            ...
            let result = match self.location.clone() {
                Location::Airbourne { x, y } => {
                    ...
                    if let Some(platform_i) =
                        self.land_stage_collision(context, action_frame, (x, y), (new_x, new_y))
                    {
                        self.y_vel = 0.0;
                        self.kb_y_vel = 0.0;
    ...
    fn check_ledge_grab(
        ...
        for (platform_i, platform) in context.surfaces.iter().enumerate() {
            ...
            if left_grab || right_grab {
                ...
                self.location = Location::GrabbedLedge {
                    platform_i,
                    d_x,
                    d_y,
                    logic: LedgeLogic::Hog,
                };
    ...
    fn check_ledge_collision(&self, ledge_grab_box: &Rect, ledge: (f32, f32)) -> bool {
        if let Location::Airbourne { x: p_x, y: p_y } = self.location {
            ...
            l_x > p_x + b_x1 && l_x < p_x + b_x2 && l_y > p_y + b_y1 && l_y < p_y + b_y2
        } else {
            false
        }
    }
    ...
    pub fn launch(
        ...
        // handle reverse hits
        let behind_entity_atk = self.bps_xy(context, action_frame, state).0
            < entity_atk.bps_xy(context).0
            && entity_atk.face_right()
            || self.bps_xy(context, action_frame, state).0 > entity_atk.bps_xy(context).0
                && !entity_atk.face_right();
        let angle = if hitbox.enable_reverse_hit && behind_entity_atk {
            PI - angle_rad
        } else {
            angle_rad
        };
        ...
        let angle = if can_di && !in_deadzone {
