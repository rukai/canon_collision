/// The contents of this module should really be in canon_collision_lib::input
/// However that would mean adding libusb as a dep to canon_collision_lib
/// So I wont move this code into canon_collision_lib until libusb has no external dependencies to setup.

use canon_collision_lib::input as canon_collision_lib_input;
        ...
        for source in &mut self.input_sources {
            match source {
                ...
                &mut InputSource::GenericController { index, ref mut state, ref mut deadzone } => {
                    ...
                    inputs.push(canon_collision_lib_input::read_generic(maps, state, events, gamepad, deadzone));
                }
            }
        }
...
fn read_gc_adapter(handle: &mut DeviceHandle<Context>, deadzones: &mut [Deadzone], inputs: &mut Vec<ControllerInput>) {
    ...
    if let Ok(_) = handle.read_interrupt(0x81, &mut data, Duration::new(1, 0)) {
        for port in 0..4 {
            ...
            let (stick_x, stick_y)     = canon_collision_lib_input::stick_filter(canon_collision_lib_input::stick_deadzone(raw_stick_x,   deadzone.stick_x),  
                                                                            canon_collision_lib_input::stick_deadzone(raw_stick_y,   deadzone.stick_y));
            let (c_stick_x, c_stick_y) = canon_collision_lib_input::stick_filter(canon_collision_lib_input::stick_deadzone(raw_c_stick_x, deadzone.c_stick_x),
                                                                            canon_collision_lib_input::stick_deadzone(raw_c_stick_y, deadzone.c_stick_y));
            let l_trigger = canon_collision_lib_input::trigger_filter(raw_l_trigger.saturating_sub(deadzone.l_trigger));
            let r_trigger = canon_collision_lib_input::trigger_filter(raw_r_trigger.saturating_sub(deadzone.r_trigger));
