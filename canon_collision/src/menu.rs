...
impl Menu {
    ...
    fn step_fighter_select(
        ...
        if let &mut MenuState::CharacterSelect {
            ref mut back_counter,
        } = &mut self.state
        {
            // animate fighters
            for selection in self.fighter_selections.iter_mut() {
                if let Some(fighter_key) = selection.fighter {
                    ...
                    if fighter.actions.contains_key(&fighter.css_action) {
                        ...
                        selection.animation_frame =
                            (selection.animation_frame + 1) % action.frames.len();
                    }
                }
            }
            ...
            for (controller_i, input) in player_inputs.iter().enumerate() {
                ...
                if input[0].stick_x < -0.7 || input[0].left {
                    if self.fighter_selections[selection_i]
                        .controller
                        .as_mut()
                        .unwrap()
                        .1
                        .tick()
                    {
                        ...
                        for (check_selection_i, selection) in
                            self.fighter_selections.iter().enumerate()
                        {
                            if check_selection_i > selection_i
                                && (selection.is_free() || check_selection_i == controller_i)
                            {
                        ...
                        if let Some(new_selection_i) = new_selection_i {
                            self.fighter_selections[new_selection_i].controller =
                                self.fighter_selections[selection_i].controller.clone();
                        ...
                        .tick()
                    {
                        ...
                        let mut new_selection_i: Option<usize> = None;
                        for (check_selection_i, selection) in
                            self.fighter_selections.iter().enumerate().rev()
                        {
                            if check_selection_i < selection_i
                                && (selection.is_free() || check_selection_i == controller_i)
                            {
                    ...
                    self.fighter_selections[selection_i]
                        .controller
                        .as_mut()
                        .unwrap()
                        .1
                ...
                if let Some((controller, _)) = selection.controller {
                    ...
                    if input.b.press {
                        match selection.ui.clone() {
                            ...
                            PlayerSelectUi::CpuTeam(_) | PlayerSelectUi::CpuAi(_) => {
            ...
            if let Some(selection_i) = remove_cpu {
                let home_selection_i = self.fighter_selections[selection_i]
                    .controller
                    .clone()
                    .unwrap()
                    .0;
                self.fighter_selections[home_selection_i].controller =
                    self.fighter_selections[selection_i].controller.clone();
    ...
    fn get_free_team(selections: &[PlayerSelect]) -> usize {
        ...
        while selections
            .iter()
            .any(|x| x.ui.is_visible() && x.team == team)
        {
            team += 1;
        }
        team
    ...
    pub fn game_setup(&mut self, package: &Package, netplay: &Netplay) {
        ...
        for (i, selection) in (&self.fighter_selections).iter().enumerate() {
            ...
            if selection.ui.is_cpu() {
                if selection.fighter.is_some()
                /* && selection.cpu.is_some() TODO */
                {
    ...
    fn step_results(&mut self, config: &Config, player_inputs: &[PlayerInput]) {
        ...
        {
            if !*replay_saved
                && (config.auto_save_replay || player_inputs.iter().any(|x| x.l.press && x.r.press))
            {
    ...
    fn step_netplay_wait(&mut self, player_inputs: &[PlayerInput], netplay: &mut Netplay) {
        ...
        let load_character =
            loading_characters[(self.current_frame / 5) % loading_characters.len()];
        ...
        match netplay.state() {
            ...
            NetplayState::MatchMaking { request, .. } => {
                self.state = MenuState::NetplayWait {
                    message: format!(
                        "Searching for online match in {} {}",
                        request.region, load_character
                    ),
                };
                if player_inputs.iter().any(|x| x.b.press) {
                    netplay.set_offline();
            ...
            NetplayState::InitConnection { .. } => {
                self.state = MenuState::NetplayWait {
                    message: format!("Connecting to peer {}", load_character),
                };
                if player_inputs.iter().any(|x| x.b.press) {
                    netplay.set_offline();
                    self.state = MenuState::GameSelect;
            ...
            NetplayState::PingTest { .. } => {
                self.state = MenuState::NetplayWait {
                    message: format!("Testing ping {}", load_character),
                };
                if player_inputs.iter().any(|x| x.b.press) {
                    netplay.set_offline();
                    self.state = MenuState::GameSelect;
            ...
            state: match self.state {
                ...
                MenuState::CharacterSelect { back_counter, .. } => {
                    RenderMenuState::CharacterSelect(
                        self.fighter_selections.clone(),
                        back_counter,
                        self.back_counter_max,
                    )
                }
                MenuState::ReplaySelect(ref replays, ref ticker) => {
                    RenderMenuState::ReplaySelect(replays.clone(), ticker.cursor)
                }
