                        ...
                        if let EntityType::Item (item) = &entity_item.ty {
                            if !item.body.is_grabbed() {
                                ...
                                if collision {
                                    ...
                                    let distance = ((item_x - player_x).powi(2) + (item_y - player_y).powi(2)).sqrt();

                                    let shortest_item = player_grabs.get(item_i).map(|x| distance < x.distance).unwrap_or(true);
                                    if shortest_item {
                                        ...
                                        player_grabs.insert(player_i, PlayerGrabHit {
                                            item_i,
                                            distance,
                                        });
                                    }
