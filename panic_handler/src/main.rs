...
use gtk::prelude::*;
use gtk::{Box, Button, Label, LinkButton, Orientation, Window, WindowType};
    ...
    match Report::from_file(file_name) {
        Ok(report) => {
            ...
            let title = match (report.payload, report.location_file, report.location_line) {
                (Some(payload), Some(file), Some(line)) => {
                    format!("{}:{} {:.100}", file, line, payload)
                }
            ...
            let address = format!(
                "https://github.com/rukai/canon_collision/issues/new?labels=panic&title={}&body={}",
                title, body
            );
            let link = LinkButton::with_label(
                &address,
                "https://github.com/rukai/canon_collision/issues/new",
            );
            if address.len() >= 2000 {
                eprintln!(
                    "This address is too long, it may not work on some browsers:\n{}",
                    address
                );
            }
            hbox.add(&link);
